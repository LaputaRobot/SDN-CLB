    ...
    def _monitor(self):
        ...
        while True:
            if not self.initRole and self.sendPath and self.initCon==0:
                ...
                for r in result:
                    ...
                    if myport in r:
                        process = r.split()[1]
                        print(process)
                        os.system('cpulimit -p {} -l 5 &'.format(process))
            ...
            if self.initRole and self.initedConNum == 4:
                ...
                print('LBR: ',LBR)
                self.respondTimelogger.info("{},{},{},{},{}".format(time.time(), self.respondTime,myLoad,LBR,self.controllerId))
                ...
                lock = f.readline()
                f.close()
                if myLoad > 100 and myLoad > avgLoad and not self.startMigration and lock == 'False' and len(otherLoads.keys())==3:
                    f = open('lock', 'w')
                    f.write('True')
                    f.close()
                    # 发现超载，向目标控制器发送迁移请求
